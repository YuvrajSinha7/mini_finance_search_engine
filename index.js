document.addEventListener('DOMContentLoaded', () => {
    ...
    const orgTitleBtn = document.getElementById('org-title');
    ...
    if (orgTitleBtn) {
        orgTitleBtn.addEventListener('click', () => {
            rightSidebar.classList.toggle('collapsed');
        });
    }

    // Handle Input change (enable/disable submit button)
    ...
    // Make suggestion cards callable from global scope (used in inline onclick)
    ...
    async function handleSendMessage() {
        ...
        // Add Shimmer (loading) to UI
        const shimmerId = addShimmerToUI();

        // Backend API Fetch
        let responseText = "Sorry, I couldn't connect to the backend server.";
        try {
            ...
            if (res.ok) {
                ...
                responseText = `Server Error: ${res.statusText}`;
            }
        } catch (error) {
            console.error("Fetch error:", error);
            responseText = "Network error. Make sure your Python backend is running!";
        }

        // Remove Shimmer
        removeMessageFromUI(shimmerId);
    ...
    function addMessageToUI(text, sender) {
        ...
        if (sender === 'user') {
            const userImageHtml = `
                <div class="user-avatar avatar-icon">
            ...
            messageWrapper.innerHTML = `
                ...
                ${userImageHtml} <!-- Avatar on the right for user -->
            `;
        } else {
            // Note: marked library is included in index.html to parse Markdown
    ...
    function addShimmerToUI() {
        ...
        messageWrapper.innerHTML = `
            ${botAvatarHtml}
            <div class="message-content bot-message" style="width: 100%; max-width: 600px;">
    ...
    function addToSidebarRecent(text) {
